...
const Rack = (props: Props) => {
  ...
  let dataProvider = new RESTDataProvider({
    keyAttributes: "id",
    url: `${API_URL}/cablingTasks/${props.ticket}/actions/getCableValidationFailureTask`,
    transforms: {
      fetchFirst: {
  ...
  const buttonClickedHandler = async () => {
    ...
    const request = new Request("http://localhost:21000/lvv/cablingTasks/" + props.ticket + "/actions/resolveValidationFailureTask", { method: "POST" });
